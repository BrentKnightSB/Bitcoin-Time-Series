...
#Reading in data
data <- read.csv("crypto-markets.csv", header=TRUE)
...
#Viewing the original data set, ACF, and PACF
ts.plot(bitcoin[,2])
acf(bitcoin[,2])
pacf(bitcoin[,2])
...
#Differencing Twice
bitcoin.diff2 <- diff(bitcoinboxcox, lag = 30)
ts.plot(bitcoin.diff2)
var(bitcoin.diff2)
...
#Using the auto arima package in the forecast library
auto.arima(bitcoin.diff)
auto.arima(bitcoin.diff2)
